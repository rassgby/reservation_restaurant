        ...
        <div class="container mx-auto px-4">
            <div class="flex flex-wrap items-center justify-between py-3">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-restaurant-gold">Dashboard Client</p>
                    </div>
                </div>
                <!-- Liens de navigation -->
                <div class="hidden md:flex items-center space-x-8">
                    <a href="{{ route('client.reservation.page') }}"
                       class="hover:text-restaurant-gold transition-colors duration-300">
                        Réserver
                    </a>
                    <a href="{{ route('client.availability.page') }}"
                       class="hover:text-restaurant-gold transition-colors duration-300">
                        Vérifier Disponibilités
                    </a>
                    <a href="{{ route('client.history.page') }}"
                       class="hover:text-restaurant-gold transition-colors duration-300">
                        Historique
                    </a>
            ...
            <div id="mobile-menu" class="hidden md:hidden pb-4">
                <div class="flex flex-col space-y-3">
                    <a href="{{ route('client.reservation.page') }}"
                       class="py-2 px-3 hover:text-restaurant-gold">Réserver</a>
                    <a href="{{ route('client.availability.page') }}"
                       class="py-2 px-3 hover:text-restaurant-gold">Vérifier Disponibilités</a>
                    <a href="{{ route('client.history.page') }}"
                       class="py-2 px-3 hover:text-restaurant-gold">Historique</a>
                    <!-- Lien mobile vers profil -->
                    <a href="{{ route('profile.edit') }}"
                       class="py-2 px-3 flex items-center space-x-2 hover:text-restaurant-gold">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                                class="py-2 px-3 hover:text-restaurant-gold flex items-center space-x-2">
